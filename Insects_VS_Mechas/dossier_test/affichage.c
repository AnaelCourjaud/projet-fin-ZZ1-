...
void creerAttaquant(combattant_t *tableauCombattants[NBRMAXCOMBATTANTS], indicesPNGs indicePNG, int indiceEmplacement){
    
    printf("début créer attaquant\n");
    combattant_t *emplacementAttaquant = malloc(sizeof(combattant_t));
    emplacementAttaquant->spriteCourant = NULL;//malloc(sizeof(spriteCourant_t));
